...
describe('toElementRef', () => {
  it('maps an element to the node kind, carrying the element itself', () => {
    const ref = toElementRef({ kind: 'element', id: 'customer' }, tree, links)!;
  ...
  it('maps a boundary to the group kind, and names what it holds', () => {
    const ref = toElementRef({ kind: 'boundary', id: 'bank' }, tree, links)!;
  ...
  it('maps a link and a relation to the edge kind, so no new kind is needed', () => {
    expect(toElementRef({ kind: 'link', id: 'api::customer' }, tree, links)?.kind).toBe('edge');
    expect(toElementRef({ kind: 'relation', id: ast.relations[0]!.id }, tree, links)?.kind).toBe(
      'edge',
    );
  });

  it('names the line a relation rides on, so an outer panel need not search', () => {
    const ref = toElementRef({ kind: 'relation', id: ast.relations[0]!.id }, tree, links)!;
  ...
  it('is null for something the model does not have', () => {
    expect(toElementRef({ kind: 'element', id: 'ghost' }, tree, links)).toBeNull();
  });
});
...
describe('fromElementRef', () => {
  it('round-trips every kind', () => {
    for (const selection of [
      ...
      expect(fromElementRef(toElementRef(selection, tree, links))).toEqual(selection);
    }
  });
